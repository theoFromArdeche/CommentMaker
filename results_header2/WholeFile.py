...
def call_LLM(prompt: str):  # Makes a POST request to the LLaMA API with the given prompt and returns the response as a string.
    data = { 
         "model": "llama3", 
         "prompt": prompt 
     }
...
def process_file(file_path, prompt_header, result_path):  # Processes a file by calling LLM with the given prompt and header, and saves the result to the specified path.
...
def main():  # Main function that parses command-line arguments, sets up the LLaMA API call and file processing.
