...
def call_LLM(prompt: str):
    # Makes a POST request to the API with the given prompt and retrieves the response.
    """Makes a POST request to the API with the given prompt and retrieves the response."""
    data = { 
        "model": "llama3", 
        "prompt": prompt 
    ...
    if response.status_code == 200:
        # Processes the response text to extract a list of JSON objects.
        """Processes the response text to extract a list of JSON objects."""
...
def process_file(file_path, prompt_header, result_path, rows_per_request):
    # Reads the file and processes it by making requests to the API.
    """Reads the file and processes it by making requests to the API."""
...
def main():
    # Parses command-line arguments and calls the process_file function for each file.
    """Parses command-line arguments and calls the process_file function for each file."""
    ...
    for file in files:
        if ("." not in file or any(s in file for s in ignore_files)):
            continue
        print(file)
