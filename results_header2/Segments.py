...
def call_LLM(prompt: str):  # Returns the response string from the LLM API.
    data = {
        "model": "llama3",
        "prompt": prompt
    }
...
def process_file(file_path, prompt_header, result_path, rows_per_request):  # Processes a file by sending it to the LLM API in chunks.
...
def main():  # Main function that parses command line arguments and processes files.
