...
private:
    ...
    bool aux(vector<vector<char>>& board, int row, int column, Trie* temp) {
        // Performs a depth-first search to find words in the trie.
        if (temp->isThisAWord) {
            // If the current node is a leaf node, add the word to the result.
...
public:
    vector<string> findWords(vector<vector<char>>& board, vector<string>& words) {
        // Finds all the words in a given trie that can be formed by characters in a given board.
        Trie* trie = new Trie();
